...
local function applySettings()
    ...
    Workspace.StreamingMinRadius = math.max(10, math.floor(settings.RenderDistance * 0.35))

    -- Camera FOV cap to help simulate FPS reduction safely
    local camera = Workspace.CurrentCamera
    if camera then
        camera.FieldOfView = clamp(80 - (settings.MaxFPS - 30) * 0.2, 50, 90)
    end

    -- Sounds
    for _, descendant in ipairs(playerGui:GetDescendants()) do
        if descendant:IsA("Sound") then
            descendant.Volume = math.clamp(descendant.Volume * settings.SoundVolume, 0, 10)
        end
    end
    ...
    for _, descendant in ipairs(Workspace:GetDescendants()) do
        if descendant:IsA("ParticleEmitter") then
            ...
            if descendant.Rate then
                descendant.Rate = math.max(0, descendant.Rate * settings.ParticleScale)
            end
    ...
    for _, descendant in ipairs(Workspace:GetDescendants()) do
        ...
        elseif descendant:IsA("Sound") then
            if settings.SoundVolume < 0.25 and removedSounds < 20 then
                ...
                descendant.Volume = math.min(descendant.Volume, settings.SoundVolume)
            end
        end
    end
...
local function addSlider(parent, labelText, key, minValue, maxValue, stepValue)
...
local function createMenu()
    ...
    for index, name in ipairs(presetNames) do
        ...
        btn.Position = UDim2.new((index - 1) / #presetNames, 4, 0, 5)
...
toggleButton.Position = UDim2.new(1, -138, 1, -74)
